...
impl MatchComposerConfig {
    pub fn from_schema(cfg: ConfigV1, log_root: PathBuf) -> Result<Self, String> {
        let server = ServerConfig {
            host: cfg.host,
            port: cfg.port,
        };

        let allies_log_root = log_root.join("allies");
        let opponents_log_root = log_root.join("opponents");
